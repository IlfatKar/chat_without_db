<template>
  <div class="home">
    ...
    <form class="inputs" @submit.prevent="send">
      ...
      <button type="button" @click="file" class="file">File</button>
...
export default {
  ...
  methods: {
    ...
    send() {
      this.lastId = uuidv4()
      const data = this.simpleCrypto.encrypt(
        JSON.stringify({
          id: this.lastId,
          text: this.input,
          sender: this.user,
  ...
  mounted() {
    ...
    this.ws = new WebSocket('ws://crystalline-calm-sleet.glitch.me/')
    this.ws.onmessage = (data) => {
      const parsed0 = JSON.parse(data.data)
      const parsed = {
        ...parsed0,
        data:
          parsed0.data && parsed0.type !== 'online'
            ? this.simpleCrypto.decrypt(parsed0.data)
            : parsed0.data,
      }
